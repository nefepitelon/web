...
const localOverlayPath = path.join(root, "grid-ops", "docs", "arcus-entropy-manual.md");
...
const localOverlay = (await readFile(localOverlayPath, "utf8")).replace(/^\uFEFF/, "").trim();
...
const arcusSectionPattern = /(^|\n)## 11\. Arcus 配置[\s\S]*?(?=\n## 12\. Robinhood Chain Lighter（RHC）配置)/;
if (!arcusSectionPattern.test(markdown)) {
  throw new Error("Unable to apply local Arcus/Entropy manual overlay: upstream Arcus section not found");
}
markdown = markdown.replace(arcusSectionPattern, `$1${localOverlay}\n`);
...
if (markdownOnly) {
  ...
  console.log(`Manual copied without HTML rendering: ${markdown.split(/\r?\n/).length} lines`);
