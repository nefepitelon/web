...
export async function POST(request: NextRequest) {
  if (!isSameOrigin(request)) return json({ error: "研报分享页仅允许从本站创建。" }, 403);
  ...
  try {
    ...
    if (!job?.reportMarkdown) {
      return persistAgentSessionOrBrowser(json({ error: `${asset.ticker} 暂无可分享的已完成研报。` }, 404), state);
    }
    ...
    const report = localized.report as unknown as Prisma.InputJsonValue;
    const share = await prisma.bstockResearchShare.upsert({
      where: {
        ownerKey_sourceJobId_language: {
      ...
      select: { shareKey: true, language: true, updatedAt: true }
    });
    const sharePath = `/bstock-alpha/report/${encodeURIComponent(share.shareKey)}`;
    return persistAgentSessionOrBrowser(json({
      shareKey: share.shareKey,
      sharePath,
      shareUrl: new URL(sharePath, request.url).toString(),
    ...
    const message = error instanceof z.ZodError
      ? "研报分享参数无效。"
      : error instanceof Error ? error.message : "研报分享页创建失败。";
    const status = walletError ? error.status : localizationError ? error.status : 502;
    const code = walletError ? error.code : localizationError ? error.code : "REPORT_SHARE_FAILED";
