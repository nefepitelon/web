import { PhoenixExchange, floorPhoenixStep } from './phoenix.js';

const FALLBACK_MARKETS = [
  { marketId: 1, name: 'SOL-PERP', displayName: 'SOL/USD', longName: 'Solana', symbol: 'SOL', exchangeSymbol: 'SOL', lastPrice: 150, stepSize: 0.01, stepPrice: 0.01, minOrderSize: 0.01, maxOrderSize: 100000, maxLeverage: 10, defaultLeverage: 3, makerFee: 0.00005, baseLotsDecimals: 2, tickSize: 1 },
  { marketId: 2, name: 'BTC-PERP', displayName: 'BTC/USD', longName: 'Bitcoin', symbol: 'BTC', exchangeSymbol: 'BTC', lastPrice: 65000, stepSize: 0.0001, stepPrice: 0.1, minOrderSize: 0.0001, maxOrderSize: 1000, maxLeverage: 10, defaultLeverage: 3, makerFee: 0.00005, baseLotsDecimals: 4, tickSize: 1000 },
  { marketId: 3, name: 'ETH-PERP', displayName: 'ETH/USD', longName: 'Ethereum', symbol: 'ETH', exchangeSymbol: 'ETH', lastPrice: 3500, stepSize: 0.001, stepPrice: 0.01, minOrderSize: 0.001, maxOrderSize: 10000, maxLeverage: 10, defaultLeverage: 3, makerFee: 0.00005, baseLotsDecimals: 3, tickSize: 10 },
];

export class PhoenixPaperExchange extends PhoenixExchange {
  constructor(opts = {}) {
    super(opts);
    this.mode = 'paper';
  ...
  async placeLimitOrder(order) {
    ...
    if (!(size >= market.minOrderSize)) throw new Error(`Phoenix 模拟单数量小于最小数量 ${market.minOrderSize}。`);
    const id = `ph-paper-${this._seq++}`;
  ...
  async _poll() {
    ...
    try {
      ...
      if (this.dataSource === 'real') {
        try { await this._pollPrices(); } catch { /* retain the last real quote */ }
      } else {
