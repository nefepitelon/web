...
export function BoxBreakoutSurface({ signedIn, canOperate }: { signedIn: boolean; canOperate: boolean }) {
  ...
  useEffect(() => { if (!signedIn) return; const abort = new AbortController(); const interval = setInterval(() => { if (!document.hidden) void poll(abort.signal); }, running ? 3000 : 60_000); return () => { clearInterval(interval); abort.abort(); }; }, [poll, running, signedIn]);
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try { const data = await request<{ quotes: Quote[]; errors?: string[] }>(`/api/box-breakout/quotes?market=${market}&symbols=${encodeURIComponent(symbolKey)}`, { signal: abort.signal }); if (!abort.signal.aborted) { setQuotes(previous => ({ ...previous, ...Object.fromEntries(data.quotes.map(quote => [`${market}:${quote.symbol}`, quote])) })); setQuoteError(data.errors?.length ? "部分报价刷新失败，继续显示上次行情及其时间。" : ""); } }
  ...
  function changeMarket(next: Market) { setMarket(next); setTopic(""); setFilter(next === "crypto" ? "all" : "qualified"); setSort(next === "crypto" ? "change" : "score"); setQuoteError(""); }
  function download() {
  ...
  return <main className={styles.surface} data-native-i18n="react">
    ...
    <div className={styles.content}>
      <header className={styles.hero}><div><div className={styles.eyebrow}><Layers3 size={14} /> BOX BREAKOUT / 箱体突破</div><h1>A股<span>&</span>加密箱体突破看板</h1><p>从震荡区间到突破信号。用热点、量价与资金，筛选值得关注的下一次共振。</p></div><div className={styles.heroActions}><button onClick={() => setHelpOpen(true)}><CircleHelp size={16} /> 策略说明</button><button onClick={() => setSettingsOpen(true)}><Settings2 size={16} /> 看板设置</button></div></header>
    ...
    {helpOpen && <Dialog title="箱体突破 · 策略与数据说明" close={() => setHelpOpen(false)}><div className={styles.help}><p>箱体采用最近 60 根日 K；发现近 15 日首次有效突破时，回看突破前箱体。至少需要 40 根有效日 K。虚线为箱顶 / 箱底，圆点为箱顶试盘。</p><h3>A 股：四项条件，各 25 分</h3><ul><li>热点共振：所属概念匹配热门 TOP 10 或自定义关注板块。</li><li>连续倍量：近 10 日最长连续 ≥3 日量比 ≥1.8，且当前量比 ≥1.8。量比以此前 5 日均量计算。</li><li>资金控盘：近 5 日主力净额为正、至少 3 日流入，并结合股东户数变化与换手率衡量控盘。</li><li>箱顶试盘：贴近箱顶、出现有效上影线并通过成交量条件，至少 3 次。</li></ul><h3>加密：三项条件，满分 100 分</h3><p>连续倍量 34 分、箱顶试盘 33 分、24h 涨幅 ≥10% 为 33 分。部分满足条件按规则给予部分分数；≥85 分为达标关注，70–84 分为突破观察。</p><h3>扫描与刷新</h3><p>全市场逐只扫描沪深股票；快速扫描先筛选量比 / 换手活跃标的，最多 200 只并保留自选。加密支持三类扫描来源：α-RadarTP 异动排行榜、α-RadarTP 热门精选主流、Binance USDT 永续涨幅 TOP 30。雷达名单只决定扫描范围，仍按箱体突破规则独立评分，不替代为雷达分数；仅分析可验证的 USDT 永续合约，跳过项会写入日志。数据源失败时保留旧结果，不使用其他榜单冒充。全市场通常需要 10–30 分钟，取决于上游限流与网络。</p><p>当前页面报价每 3 秒请求更新，日 K 图表缓存 60 秒。自动扫描为工作日北京时间指定时刻，不自动识别法定休市日；默认关闭，须在设置中开启。定时任务与通知均可随时关闭。</p><h3>来源与边界</h3><p>行情来自东方财富、腾讯 / 新浪与 Binance 公开接口。资金或股东披露有滞后，缺失项会显示数据说明；不会把无数据当作零值或真实信号。</p><p>参考 <a href="https://github.com/Theclues/TradeGenuis-box" target="_blank" rel="noreferrer">TradeGenuis-box 的功能与规则 ↗</a>，由本项目独立实现原生界面、计算和持久任务。未嵌入第三方页面，也不会自动下单。</p></div></Dialog>}
  </main>;
}
