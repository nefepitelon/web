...
import { maskProxy } from "@/lib/tidesight/execution/credentials";

export async function getOrCreateTideSightExecutionConfig(userId: string) {
  return prisma.tideSightExecutionConfig.upsert({
    where: { userId },
    update: {},
    create: { userId }
  });
...
export async function tideSightCredentialSummary(userId: string) {
  const credentials = await prisma.tideSightTradingCredential.findMany({
    where: { userId },
    select: {
      id: true,
      environment: true,
...
export async function writeTideSightAudit(input: {
  userId: string;
  ...
  return prisma.tideSightTradingAudit.create({
    data: {
      userId: input.userId,
      state: input.state,
      status: input.status,
...
export function modeFrom(value: string) {
  ...
  if (normalized === "PAPER") return AlphaExecutionMode.PAPER;
  throw new Error("TideSight 仅支持 PAPER / LIVE");
}
...
export function publicConfig(config: Awaited<ReturnType<typeof getOrCreateTideSightExecutionConfig>>) {
  return {
    ...
    autoRunId: config.autoRunId,
    autoStartedAt: config.autoStartedAt?.toISOString() ?? null,
    autoHeartbeatAt: config.autoHeartbeatAt?.toISOString() ?? null,
    autoError: config.autoError,
    ...
    minTideSightScore: config.minTideSightScore,
...
export async function executionSnapshot(userId: string) {
  const [config, credentials, plans, orders, positions, audits, closedPositionTotals] = await Promise.all([
    getOrCreateTideSightExecutionConfig(userId),
    tideSightCredentialSummary(userId),
    prisma.tideSightExecutionPlan.findMany({
      where: { userId },
      orderBy: { createdAt: "desc" },
      take: 20,
      include: { intent: { select: { symbol: true, side: true, source: true, tideSightScore: true } } }
    }),
    prisma.tideSightTradingOrder.findMany({ where: { userId }, orderBy: { createdAt: "desc" }, take: 40 }),
    prisma.tideSightTradingPosition.findMany({
      where: { userId },
      orderBy: { openedAt: "desc" },
      take: 40,
      include: { plan: { select: { intent: { select: { source: true, tideSightScore: true, leverage: true } } } } }
    }),
    prisma.tideSightTradingAudit.findMany({ where: { userId }, orderBy: { createdAt: "desc" }, take: 60 }),
    prisma.tideSightTradingPosition.groupBy({
