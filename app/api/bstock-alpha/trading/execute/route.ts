...
async function latestAgentStudioScore(ownerKey: string, agentKey: string, ticker: string) {
  try {
    ...
    return agentStudioRatingScore(extractAgentStudioReportSummary(report.reportMarkdown).rating);
  } catch {
    return null;
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const studioScore = await latestAgentStudioScore(ownerKey, sessionKey, asset.ticker);
    ...
    try {
      await prisma.bstockTradeRecord.update({
        where: { intentHash },
        data: { ownerKey, agentKey: sessionKey, status: "SUBMITTING" }
      });
    } catch (error) {
    ...
    const response = json({ error: known ? error.message : message, code: known ? error.code : "TRADE_EXECUTION_FAILED" }, known ? error.status : 502);
