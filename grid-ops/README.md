...
**第一步：选交易对**。“全部交易对”下拉框列出该所全部可交易市场（如 BTC/USD、ETH/USD）。“推荐交易对”支持选择中性、做多或做空策略，并用 10% 一档的“最低强度”设定筛选门槛；默认是中性网格且强度严格大于 30%。点击“分析”后，程序会按当前 K 线周期扫描全部开放市场；点击推荐项即可切换交易对。扫描只读取公开行情，不会下单。
...
升级线上网页不会自动替换已运行的本地引擎。请保留 `.env` 与 `data`，通过平台下载最新引擎或使用启动器更新，在合适的维护时间重启并确认版本 **2.3.3**；无需重填交易密钥。本次回归使用公开市场规则与离线签名/模拟接口，不会发出实盘测试订单。
