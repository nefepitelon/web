...
async function enrichCandidate(coin) {
  ...
  const klines = fulfilled(results[0], []);
  const klineAvailable = Array.isArray(klines) && klines.length >= 20;
  const klineMetrics = analyzeKlines(klines);
  ...
  return { ...coin, ...klineMetrics, klineAvailable, openInterestChange, longShortRatio };
}
...
function buildReason(coin, signalType) {
  const pieces = [
    coin.klineAvailable ? `15m ${coin.return15m >= 0 ? "+" : ""}${coin.return15m.toFixed(1)}% / 4h ${coin.return4h >= 0 ? "+" : ""}${coin.return4h.toFixed(1)}%` : "短周期 K 线暂缺",
    coin.klineAvailable ? `小时量能 ${coin.volumeMultiple.toFixed(1)}×` : "小时量能暂缺",
    coin.openInterestChange == null ? coin.perp ? "OI 数据暂缺" : "现货不适用 OI" : `OI ${coin.openInterestChange >= 0 ? "+" : ""}${coin.openInterestChange.toFixed(1)}%`,
    coin.fundingAvailable ? `资金费率 ${coin.fundingPercent >= 0 ? "+" : ""}${coin.fundingPercent.toFixed(3)}%` : "资金费率暂缺"
  ];
...
// Shared read-only scanner: the radar page and Box Breakout use identical universes/rules.
export async function buildAlphaScanSnapshot() {
    ...
    const universe = [...symbols].map((symbol) => {
      ...
      return {
        ...
        fundingAvailable: premium?.lastFundingRate != null && Number.isFinite(Number(premium.lastFundingRate)),
    ...
    const scoredUniverse = enriched.map((coin) => {
      ...
      const dimensionAvailability = [coin.klineAvailable, coin.klineAvailable, coin.fundingAvailable, coin.longShortRatio != null, coin.klineAvailable && coin.openInterestChange != null, sourceResults[4].status === "fulfilled", feeds.length > 0];
      ...
      const autogeneratedHeadlines = coin.klineAvailable ? [
      ...
      ] : [`${coin.symbol} 24H 价格变化 ${coin.change24h.toFixed(1)}%`, "短周期 K 线暂缺，等待市场数据源恢复"];
      const risks = [];
      if (dimensionAvailability.some((available) => !available)) risks.push("部分维度暂缺，综合分包含模型基线；请等待数据补齐");
      ...
      return {
        ...
        volume: coin.klineAvailable ? Number(coin.volumeMultiple.toFixed(2)) : null,
        funding: coin.fundingAvailable ? Number(coin.fundingPercent.toFixed(4)) : null,
        fundingAvailable: coin.fundingAvailable,
        ...
        watched: false,
        dimensions: dimensions.map((value, index) => dimensionAvailability[index] ? value : null),
        dimensionAvailability,
        reason: buildReason(coin, interpreted.signalType),
        heat: dimensionAvailability[5] ? socialScore : null,
        kols: coin.trendingRank ? [`CoinGecko Trending #${coin.trendingRank}`] : [dimensionAvailability[5] ? "公开趋势未入榜" : "公开趋势源暂缺"],
        risks,
        headlines: matchedHeadlines.length ? matchedHeadlines.slice(0, 2) : autogeneratedHeadlines,
        metrics: {
          return15m: coin.klineAvailable ? Number(coin.return15m.toFixed(2)) : null,
          return1h: coin.klineAvailable ? Number(coin.return1h.toFixed(2)) : null,
          return4h: coin.klineAvailable ? Number(coin.return4h.toFixed(2)) : null,
          volumeZ: coin.klineAvailable ? Number(coin.volumeZ.toFixed(2)) : null,
          atrPercent: coin.klineAvailable ? Number(coin.atrPercent.toFixed(2)) : null,
          breakout: coin.klineAvailable ? coin.breakout : null,
...
export default async function handler(request, response) {
  ...
  try {
    const snapshot = await buildAlphaScanSnapshot();
