...
const ENGINE_VERSION = "tidesight-execution-v1.2-auto-risk-cap";
const EXECUTION_MODES = new Set(["paper", "live"]);
const BINANCE_MODES = new Set(["live", "testnet"]);
const DEFAULT_POLICY = Object.freeze({
  riskPerTradePct: 0.75,
  maxRiskPerTradePct: 1.5,
  maxLeverage: 3,
  absoluteMaxLeverage: 25,
  dailyLossLimitPct: 2,
...
function resolvePolicy(input = {}) {
  return {
    ...
    maxPortfolioExposurePct: clamp(finiteNumber(input.maxPortfolioExposurePct, DEFAULT_POLICY.maxPortfolioExposurePct), 5, 75),
    ...
    minTideSightScore: clamp(finiteNumber(input.minTideSightScore, DEFAULT_POLICY.minTideSightScore), 50, 95),
...
function normalizeIntent(input = {}, policy = DEFAULT_POLICY, now = new Date()) {
  ...
  return {
    ...
    riskPct,
    requestedNotional: finiteNumber(input.requestedNotional),
    source: String(input.source || "manual").trim().slice(0, 80),
    tideSightScore: finiteNumber(input.tideSightScore),
...
function hardRiskChecks(intent, context, policy, now) {
  ...
  const canCapAutomaticNotional = context.trustedMacdSignal === true && context.riskCappedSizing === true;

  if (!EXECUTION_MODES.has(intent.mode)) violations.push({ code: "MODE_INVALID", message: "执行模式必须为 paper 或 live。" });
  ...
  if (!(intent.riskPct > 0) || intent.riskPct > Math.min(policy.riskPerTradePct, policy.maxRiskPerTradePct)) {
  ...
  if (intent.entryPrice > 0 && intent.stopLoss > 0 && intent.takeProfit > 0 && intent.side) {
    ...
    if (stopDistancePct >= 80 / intent.leverage) {
      violations.push({ code: "LIQUIDATION_BUFFER", message: "止损距离超出保守杠杆保证金缓冲，拒绝开仓。" });
    }
    if (intent.requestedNotional != null) {
      const requested = intent.requestedNotional;
      const riskWithCosts = requested * (stopDistancePct + 0.15) / 100;
      const tolerance = 1e-8;
      const exceedsRisk = riskWithCosts > equity * intent.riskPct / 100 + tolerance;
      const exceedsLimits = requested > policy.perOrderNotionalLimit + tolerance
        || requested + finiteNumber(context.dailyExecutedNotional, 0) > policy.dailyNotionalLimit + tolerance
        || requested + finiteNumber(context.openNotional, 0) > equity * policy.maxPortfolioExposurePct / 100 + tolerance;
      const exceedsMargin = requested * (1 / intent.leverage + 0.0015) > finiteNumber(context.availableBalance, equity) + tolerance;
      if (!(requested >= 5)) violations.push({ code: "FIXED_NOTIONAL_RISK", message: "目标名义仓位不得小于 5 USDT。" });
      if (exceedsRisk) {
        const issue = { code: canCapAutomaticNotional ? "AUTO_NOTIONAL_RISK_CAPPED" : "FIXED_NOTIONAL_RISK", message: canCapAutomaticNotional ? "自动策略目标名义仓位将按单笔净值风险额度安全下调。" : "固定名义仓位的止损风险（含 0.15% 费用/滑点预算）超过单笔净值风险额度。" };
        (canCapAutomaticNotional ? warnings : violations).push(issue);
      }
      if (exceedsLimits) {
        const issue = { code: canCapAutomaticNotional ? "AUTO_NOTIONAL_LIMIT_CAPPED" : "FIXED_NOTIONAL_LIMIT", message: canCapAutomaticNotional ? "自动策略目标名义仓位将按单笔、单日及组合剩余额度安全下调。" : "固定名义仓位超过单笔、单日或组合额度；不自动缩放。" };
        (canCapAutomaticNotional ? warnings : violations).push(issue);
      }
      if (exceedsMargin) {
        const issue = { code: canCapAutomaticNotional ? "AUTO_NOTIONAL_MARGIN_CAPPED" : "MARGIN_INSUFFICIENT", message: canCapAutomaticNotional ? "自动策略目标名义仓位将按可用保证金安全下调。" : "可用保证金不足（含费用与统一账户安全缓冲）。" };
        (canCapAutomaticNotional ? warnings : violations).push(issue);
      }
    }
    if (stopDistancePct + 1e-9 < policy.minStopDistancePct) {
    ...
    if (riskRewardRatio + 1e-9 < policy.minRiskRewardRatio) {
      violations.push({ code: "RISK_REWARD_TOO_LOW", message: `风险回报比不得低于 1:${policy.minRiskRewardRatio}。` });
    }
  }

  if (intent.tideSightScore == null && context.trustedMacdSignal !== true) violations.push({ code: "SCORE_MISSING", message: "普通策略必须提供独立 TideSight 评分。" });
  else if (intent.tideSightScore != null && intent.tideSightScore < policy.minTideSightScore) violations.push({ code: "SCORE_BELOW_THRESHOLD", message: `TideSight 分 ${intent.tideSightScore} 低于阈值 ${policy.minTideSightScore}。` });
...
function buildExecutionPlan(intent, context, policy, equity, now) {
  ...
  const riskQuantity = requestedRiskAmount / (stopDistance + intent.entryPrice * 0.0015);
  const riskRemainingNotional = Math.max(0, riskQuantity * intent.entryPrice);
  ...
  const marginRemainingNotional = Math.max(0, finiteNumber(context.availableBalance, equity)) / (1 / intent.leverage + 0.0015);
  const remainingNotional = Math.min(riskRemainingNotional, portfolioRemainingNotional, policy.perOrderNotionalLimit, dailyRemainingNotional, marginRemainingNotional);
  const requestedNotional = intent.requestedNotional ?? riskQuantity * intent.entryPrice;
  const notional = Math.min(requestedNotional, remainingNotional);
  const quantity = notional / intent.entryPrice;
  const actualRiskAmount = quantity * (stopDistance + intent.entryPrice * 0.0015);
  ...
  return {
    ...
    risk: {
      accountMode: context.portfolioMargin ? "portfolio" : "classic",
      portfolioMargin: context.portfolioMargin || null,
      ...
      targetNotional: round(requestedNotional, 2),
      riskRemainingNotional: round(riskRemainingNotional, 2),
      portfolioRemainingNotional: round(portfolioRemainingNotional, 2),
      marginRemainingNotional: round(marginRemainingNotional, 2),
      marginRequired: round(notional / intent.leverage, 2),
      leverage: intent.leverage,
      exposureCapped: notional + 0.000001 < requestedNotional,
      sizingMode: context.trustedMacdSignal === true && context.riskCappedSizing === true ? "RISK_CAPPED_TARGET" : "FIXED_OR_RISK",
