...
支持 Decibel（Aptos 链）、Extended（Starknet 链）、RISEx、Binance、Ondo Perps、Phoenix、Nado、OKX、GRVT、RHC Lighter 与 Arcus 等多家交易所；Entropy 当前仅支持官方实时行情驱动的 `paper` 模拟，不开放 `live`
多个交易所可以同时各跑一个网格策略，统一在一个浏览器仪表盘里监控和操控
如果你还尚未注册交易所，可以用在金的邀请链接，万分感谢🙏
Decibel 注册链接：https://app.decibel.trade/r/C5WV3H
Extended 注册链接：https://app.extended.exchange/join/WELINKBTC
Arcus 注册链接：https://app.arcus.xyz/ref/WELINKBTC
RHC Lighter 注册链接：https://robinhoodchain.lighter.xyz/?referral=WELINKBTC
...
`paper` 使用程序内部的模拟余额、模拟持仓和模拟订单，不会向交易所发送真实订单。程序会尽量读取交易所公开实时行情；行情接口不可用时，部分适配器可能退回合成行情。每个交易所的模拟账户各自以 `PAPER_BALANCE` 为初始余额，因此多个交易所都开模拟盘时，不代表你真实拥有多倍资金。
...
| `PORT` | `8080` | `1-65535` 的整数。被占用时可改为 `8081` 等未占用端口。 |
| `PAPER_BALANCE` | `10000` | 每个模拟交易所各自的初始 USDC 余额，必须大于 0。 |

访问地址由 `HOST` 和 `PORT` 组成，例如 `http://127.0.0.1:8080`。
...
官方入口：https://app.decibel.trade/r/C5WV3H
...
- [Extended 交易界面](https://app.extended.exchange/join/WELINKBTC)
...
- [RHC Lighter 应用](https://robinhoodchain.lighter.xyz/?referral=WELINKBTC)
...
顶部显示多个交易所的 `PAPER/LIVE` 模式、连接点、代理状态、本地后端连接和时间。
...
“全部交易对”来自交易所当前返回的活跃市场。“K线周期”可选 `15m / 1h / 4h / 1d`。“推荐交易对”可选择中性、做多或做空，最低强度按 10% 一档选择；默认筛选中性网格且强度严格大于 30%。点击“分析”会按当前周期扫描全部开放市场，点击推荐项即可切换。该分析只读取行情，不会下单。趋势算法使用 EMA20、EMA50、最近 20 根归一化斜率和 ATR14：
...
Invoke-RestMethod -Method Post -Uri 'http://127.0.0.1:8080/api/de/start' -ContentType 'application/json' -Body $body
```
...
| 方法 | 路径 | 作用 |
|---|---|---|
| GET | `/api/overview` | 全部交易所摘要 |
| GET | `/api/overview/stream` | 每秒一次的全部交易所完整 SSE 状态 |
...
- `src/exchange/`：多个交易所的 live/paper 适配器；
...
保持启动窗口开启，手动访问 `.env` 中的地址，默认是 `http://127.0.0.1:8080`。若页面仍打不开，查看窗口是否显示“已启动”，并检查端口错误。
