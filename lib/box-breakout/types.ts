...
export type CryptoScanMode = "crypto" | "crypto-radar" | "crypto-mainstream" | "crypto-risk-pool" | "crypto-alpha-market-cap" | "crypto-alpha-open-interest";
export type ScanMode = "market" | "quick" | "pool" | CryptoScanMode;
export function isCryptoScanMode(mode: ScanMode): mode is CryptoScanMode {
  return mode === "crypto" || mode === "crypto-radar" || mode === "crypto-mainstream" || mode === "crypto-risk-pool"
    || mode === "crypto-alpha-market-cap" || mode === "crypto-alpha-open-interest";
}
...
export interface ScanJob { id: string; mode: ScanMode; status: "queued" | "running" | "complete" | "cancelled" | "failed"; total: number; processed: number; qualified: number; errors: number; startedAt: string; completedAt: string | null; logs: string[]; sourceSymbols?: Stock[] }
...
export type Command = { action: "scan"; mode: ScanMode; symbols?: string[] } | { action: "cancel" } | { action: "pool-add"; symbol: string; name?: string } | { action: "pool-remove"; symbol: string } | { action: "settings"; sectors?: string[]; auto?: boolean; autoTimes?: string[]; telegramEnabled?: boolean; telegramChat?: string; telegramToken?: string } | { action: "telegram-test" };
